...
while (tecla < 0):
    ...
    esqueleto = cv.CloneImage(branca)
    ...
    if user.users != []:
      ...
      if skel_cap.is_tracking(id):
          for membro in membros:
            points[membro] = []
            for junta in membros[membro]:
              points[membro].append(skel_cap.get_joint_position(id, junta).point)

          for membro in points:
            points[membro] = depth.to_projective(points[membro])
            for i in xrange(len(points[membro])):
